...
\begin{algorithm}[H]
\SetAlgoLined
\KwData{$w, t = 1, \alpha \leq \frac{1}{2}$}
\While{$||\Phi(w + \Delta w)||> ||\Phi(w) + \alpha t \nabla \Phi(w)^T \Delta w||$}{
    $t = \beta t$
\EndWhile}
\KwRet{$w + t\alpha \Delta w, $}
...
\SetAlgoLined
\KwData{$w, \mu, g = []$}
\While {$||\nabla \mathcal{L}||$ or $\max (0, h) \geq$ tol}{
    Formulate SQP (equation \ref{eq:SQP_problem})\\
    Solve for $\Delta w$ (equation \ref{eq:active_set_solve_SQP}, $h_A = h$)\\
    Perform line search with merit function to update $w, \mu$\\
    (equation \ref{alg:Armijos_backtrack}, \Phi = T_1)\\
\EndWhile}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=.8\textwidth]{pythonProject/Figures/SQO}
