...
def Single_Shooting_RK4(param, method='IPOPT'):
    if param == 'Social_Distancing':
        from Parameters.Parameters_Social_Distancing import x0, u_min, u_max, f, X_plot, Q_plot, N, M, tgrid_M
    elif param == 'Vaccination':
        from Parameters.Parameters_Vaccination_Flat import x0, u_min, u_max, f, X_plot, Q_plot, N, M, tgrid_M
    elif param == 'Isolation':
        from Parameters.Parameters_Isolation import x0, u_min, u_max, f, X_plot, Q_plot, N, M, tgrid_M
    else:
        return
    ...
    solver(x0=[u_max] * N, lbx=[u_min] * N, ubx=[u_max] * N, lbg=[], ubg=[])
    ...
    with open(parent + '/data/Single_Shooting_' + method + '_' + param + '.pck', 'wb') as file:
